...
const iTri = s => {
  ...
  const distance = option.Swim + option.Bike + option.Run;

  if (s === 0) return 'Starting Line... Good Luck!';
  else if (s >= distance) return "You're done! Stop running!";
  else if (s >= distance - 10) return { Run: 'Nearly there!' };
  else {
    for (let key in option) {
      console.log(option[key]);
      if (s < option[key])
        return { [key]: `${(distance - s).toFixed(2)} to go!` };
      else if (option.Swim + option.Bike < s)
